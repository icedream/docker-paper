...
NEW_PAPER_URL="https://ci.destroystokyo.com/job/Paper-1.12-Pre-release/${NEW_PAPER_BUILD}/artifact/paperclip-${NEW_PAPER_BUILD}.jar"

curl -I -f "${NEW_PAPER_URL}" || (echo "Version not found." && exit 1)

commitmessage="Update to Paper 1.12 pre-release build $1"
...
sed -i 's,`mc-1.12-pre-b'"$PAPER_BUILD"'`,`mc-1.12-pre-b'"$NEW_PAPER_BUILD"'`,' README.md
git add README.md

git commit -m "$commitmessage."
git tag -s -m "Paper 1.12 pre-release server build $1." "mc-1.12-pre-b$1"
